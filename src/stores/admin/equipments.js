...
export const useAdminEquipmentsStore = defineStore("adminEquipments", {
  state: () => ({
    list: [],
    detail: null,
    count: 0,
  ...
  actions: {
    async loadList(filters={}) {
      ...
      let response = await sendRequest(
        urls.EQUIPMENTS.ADMIN.EQUIPMENTS.LIST,
        {},
        {},
        filters
      )
    ...
    async loadDetail(id) {
      ...
      this.detail = await sendRequest(
        urls.EQUIPMENTS.ADMIN.EQUIPMENTS.DETAIL,
        {},
        { id }
      )
    ...
    async deleteItem(id) {
      ...
      try {
        await sendRequest(
          urls.EQUIPMENTS.ADMIN.EQUIPMENTS.DELETE,
          {},
          { id: id }
        )
      } catch (e) {
    ...
    async createItem(data) {
      ...
      try {
        await sendRequest(
          urls.EQUIPMENTS.ADMIN.EQUIPMENTS.CREATE,
          data
        )
      } catch (e) {
        throw e
    ...
    async updateItem(id, data) {
      ...
      try {
        await sendRequest(
          urls.EQUIPMENTS.ADMIN.EQUIPMENTS.UPDATE,
          data,
          { id: id }
        )
      } catch (e) {
