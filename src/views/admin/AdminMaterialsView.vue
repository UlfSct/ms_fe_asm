...
const store = useAdminMaterialsStore()
...
<template>
  ...
  <v-card v-else class="ma-3">
    <v-toolbar flat density="comfortable">
      <v-toolbar-title>Список глобальных материалов</v-toolbar-title>
      <v-text-field
      ...
      />
      <v-tooltip text="Создать материал" location="bottom">
    ...
    >
      ...
      <template v-slot:item.base_color="{ item }">
        <div :style="`background: ${item.base_color}; border: 1px solid black; color: ${Number(item.base_color[1]) < 8 ? 'white' : 'black'}`">
          {{ item.base_color }}
        </div>
      </template>
      ...
      <template v-slot:item.actions="{ item }">
        <v-tooltip text="Редактировать материал" location="bottom">
          <template v-slot:activator="{ props }">
            <v-btn
              v-bind="props"
              size="30"
        ...
        <v-tooltip v-if="item.can_delete" text="Удалить материал" location="bottom">
          <template v-slot:activator="{ props }">
            <v-btn
              v-bind="props"
              size="30"
  ...
  <admin-create-update-material-dialog
  ...
  <apply-dialog
    ...
    text="Вы уверены что хотите удалить данный материал?"
    :loading="loadingDelete"
    @cancel="closeDeleteDialog(false)"
    @submit="closeDeleteDialog(true)"
  />
  <admin-material-info-dialog
