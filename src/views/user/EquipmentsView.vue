...
<template>
  <v-card class="ma-3">
    <v-toolbar flat density="comfortable">
      <v-toolbar-title>Список доступного оборудования</v-toolbar-title>
      <v-text-field
    ...
    >
      ...
      <template v-slot:item.model="{ item }">
        {{ item.model.name }}
        <a :href="item.model.file" download>
      ...
      <template v-slot:item.actions="{ item }">
        <v-tooltip v-if="!item.is_global && item.is_mine" text="Редактировать оборудование" location="bottom">
          <template v-slot:activator="{ props }">
            <v-btn
              v-bind="props"
              size="30"
        ...
        <v-tooltip v-if="item.can_delete && !item.is_global && item.is_mine" text="Удалить оборудование" location="bottom">
          <template v-slot:activator="{ props }">
            <v-btn
              v-bind="props"
              size="30"
        ...
        <td colspan="7">
          <v-card class="ma-3">
            <v-toolbar flat density="comfortable">
              ...
              <v-spacer />
              <v-tooltip
                v-if="rowProps.item.can_delete && !rowProps.item.is_global && rowProps.item.is_mine"
                text="Создать штуцер"
                location="bottom"
              >
                <template v-slot:activator="{ props }">
            ...
            >
              ...
              <template v-slot:item.actions="{ item }">
                <v-tooltip v-if="rowProps.item.can_delete && !rowProps.item.is_global && rowProps.item.is_mine" text="Редактировать штуцер" location="bottom">
                  <template v-slot:activator="{ props }">
                    <v-btn
                      v-bind="props"
                      size="30"
                ...
                <v-tooltip v-if="rowProps.item.can_delete && !rowProps.item.is_global && rowProps.item.is_mine" text="Удалить штуцер" location="bottom">
                  <template v-slot:activator="{ props }">
                    <v-btn
                      v-bind="props"
                      size="30"
  ...
  <create-update-equipment-dialog
